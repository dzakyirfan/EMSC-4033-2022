...
def basin_scatterplot(array):
    ...
    plt.savefig('3d_basin.png', dpi=800)
...
def latitudinal_longitudinal_contourplot(dataframe, coordinate_type):
    ...
    plot = ax.tricontourf(x, y, vel, levels=levels, cmap='RdBu')
    ...
    cbar6 = plt.colorbar(plot)
    cbar6.ax.set_ylabel('Shear wave (km/s)')

    if coordinate_type == 'Latitude':
        plt.savefig('north_south.png', dpi=800)
    elif coordinate_type == 'Longitude':
        plt.savefig('east_west.png', dpi=800)
    else:
        raise Exception("Could not generate any picture")
